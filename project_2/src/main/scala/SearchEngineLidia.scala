...
class SearchEngine {
  ...
  // Paths
  val OUTPUT_PATH: String         = "output.txt"
...
object SearchEngine {
  def main(args: Array[String]): Unit = {
    ...
    for (q <- obj.queries) {
      ...
      val docRanks = BiGramLanguageModeler(obj)
  ...
  def ScoreModeler(obj: SearchEngine): Unit = {

  }

  def LanguageModeler(obj: SearchEngine): MHashMap[String, Double] = {
  ...
  def UniGramLanguageModeler(obj: SearchEngine): MHashMap[String, Double] = {
  ...
  def BiGramLanguageModeler(obj: SearchEngine): MHashMap[String, Double] = {
  ...
  def QueryInvIndex(obj: SearchEngine): MHashMap[String, ListBuffer[(String, Int)]] = {
  ...
  def tipsterParse(obj: SearchEngine): ListBuffer[DocToTokensMap] = {
  ...
  def tipsterParseTest(obj: SearchEngine): ListBuffer[DocToTokensMap] = {
    var docTokens: ListBuffer[DocToTokensMap] = ListBuffer.empty[DocToTokensMap]
  ...
  def createDocToTokensMap(doc: Document): DocToTokensMap = {
    ...
    def words: List[String] = doc.tokens
    ...
    // new DocToTokensMap(doc.name, wordsWithoutSW.groupBy(identity).
    //mapValues(_.size).filter(p => p._2 > 1).toSeq) //

     new DocToTokensMap(doc.name, porterStemmedList.groupBy(identity).
      mapValues(_.size).filter(p => p._2 > 1).toSeq) //
  }
